...
MODEL_LIST = [
    ...
    "medsam1", "vit-google", "medmamba", "clip"
]

def model_Loader(model_name: str, num_classes: int, type: str):
    """ 
    ...
    if model_name in MODEL_LIST:
        ...
        # 모듈에서 함수 가져오기
        # if outlayer_num > 1: #Multi Model
        #     model_fn = getattr(module, "multi_model")
        # elif outlayer_num == 1: # Binary Model
        #     model_fn = getattr(module, "binary_model")
        # else: 
        #     raise ValueError(f"outlayer_num must be greater than 0.")
        model_fn = getattr(module, "model_setup")
        
        print(f"Model '{model_name}' loaded.")
        
        return model_fn(type, num_classes)  # 'type'을 전달하여 모델 인스턴스 반환
