...
class model_setup(nn.Module):
    """ 
        Ref : https://github.com/YubiaoYue/MedMamba
        Cervical-US Pretrained Model
    """
    def __init__(self, type:str, num_classes:int):
        super(model_setup, self).__init__()
        self.num_classes = num_classes
        self.base_model = VSSM(depths=[2, 2, 4, 2],dims=[96,192,384,768],num_classes=4).to(device)
        self.base_model.load_state_dict(torch.load('/home/eiden/eiden/PCOS-roi-classification/checkpoints/Medmamba/MedMamba.pth', map_location='cuda'))
        self.base_model.head = nn.Linear(in_features=768, out_features=num_classes, bias=True)
        print(f"Precise MedMamba Loaded")
